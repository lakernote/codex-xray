...
执行解剖会从用户主动选择的原始 Session 按需读取并显示用户消息、助手消息和可读摘要。SQLite 只保存还原真实调用过程所需的结构化聚合，不保存这些消息正文、完整补丁、读取到的文件内容或完整工具输出。为支持执行学习，它会保存限长且脱敏的命令、脚本和参数摘要。
...
7. **LLM 事件**：每个 `token_count` 作为一次模型返回的 Token 结算记录并编号，展示模型、未缓存输入、缓存输入、输出、推理输出、总量、上下文窗口、相对上次调用的上下文变化、缓存命中率和当次 API 等价成本。用户消息、assistant `commentary/final_answer` 和 Session 中存在的可读 Reasoning 摘要会从原始文件按需显示，但不进入 SQLite；加密 Reasoning 只能显示记录存在与大小，无法还原正文。
...
- 不读取 `auth.json`；
- 会从用户选择的原始 Session 按需显示提示词、回复正文和可读摘要，但不把这些正文写入 SQLite 索引；
- 不保存邮箱、完整补丁、读取到的文件内容或完整工具输出；
